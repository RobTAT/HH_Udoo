...
float baselineMatrix [MAX_FRAME_NUMBER];
int Frame_ctr = 0;
int para_ctr = 0;
...
int main(int argc, char** argv) {
	...
	while (((float)(std::chrono::duration_cast<std::chrono::nanoseconds>(base_timeline_e - base_timeline_s).count()))<=(float)(100000000)){
		base_timeline_e = std::chrono::high_resolution_clock::now();
	}
	baselineMatrix[Frame_ctr-1] = (float)(std::chrono::duration_cast<std::chrono::nanoseconds>(base_timeline_e-base_timeline_s).count())/((float)ms);
	cout << "=========== timeline Interval: " << baselineMatrix[Frame_ctr-1] << "=================" << endl;
